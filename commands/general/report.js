    ...
    if (config.Other.HelpMenu == 'categorized') {
      ...
      function sendHelpMenu() {
        message.channel.send(help).then(async msg => {
          if (modules.general && modules.general.enabled == true && CommandList.normal.general.length > 0) await msg.react('馃檪');
          if (modules.tickets && modules.tickets.enabled == true && CommandList.normal.tickets.length > 0) await msg.react('馃師锔�');
          if (modules.coins && modules.coins.enabled == true && CommandList.normal.coins.length > 0) await msg.react('馃挵');
          if (modules.exp && modules.exp.enabled == true && CommandList.normal.exp.length > 0) await msg.react('鉁�');
          if (modules.fun && modules.fun.enabled == true && CommandList.normal.fun.length > 0) await msg.react('馃幃');
          if (modules.minecraft && modules.minecraft.enabled == true && CommandList.normal.minecraft.length > 0) await msg.react('鉀忥笍');
          if (modules.music && modules.music.enabled == true && CommandList.normal.music.length > 0 && (musicAddon || ultimateMusicAddon)) await msg.react('馃幍');
          if (modules.other && modules.other.enabled == true && CommandList.normal.other.length > 0) await msg.react('馃梻锔�');
        });
      }
...
// 239232   8501   2229706    63250   1613689679   NULLED BY 0xEB   2229706 1613689679   NULLED BY 0xEB   2229706